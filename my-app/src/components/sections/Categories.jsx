    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
        {categories.map((cat, i) => (
          <motion.button
            key={cat.title}
            aria-label={`View category ${cat.title}`}
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5, delay: i * 0.1 }}
            viewport={{ once: true }}
            className="relative group rounded-2xl overflow-hidden shadow-lg cursor-pointer outline-none focus:ring-4 focus:ring-primary/40"
          >
            <img
              src={cat.img}
              alt={cat.title}
